...
class iCIFAR10(data.Dataset):
    ...
    def __init__(self, root, dataset, num_labeled=None, num_unlabeled_pt=0,
        ...
        self.root = os.path.expanduser(root) # home directory
        ...
        if self.ul_dist == 'rand': # not use
            ...
            # Use this when l_dist is vanilla
        ...
        if self.l_dist == 'super':
            ...
            for super_k in np.unique(self.course_targets):  # super_k = super-class idx -> 1~20
                ind_task = np.where(self.course_targets == super_k)[0] # 각 super-class에 해당하는 data index
                ind_task_labels = [self.targets[ind_task[i]] for i in range(len(ind_task))] # Target index corresponding to each test
                classes_in_task = np.unique(ind_task_labels) # Unique targets for each task / super-class에 할당된 5개의 unique class
                self.tasks.append(classes_in_task.tolist())
                if self.ul_dist == 'super' or self.ul_dist == 'neg': # not use
                    valid_ul_ = []
                    for super_k_embedded in np.unique(self.course_targets): #super_k_embedded = super-class idx -> 1~20
        ...
        # if validation data
            ...
            if self.train:
                ...
                # split dataset
                self.archive = [] # labeld dataset(data, target) -> [20, 5 ,500]
                ul_data = []
                ul_targets = []
